...
inventors.filter(inventor => inventor.year >= 1500 && inventor.year < 1600).forEach(inventor => veryOldInventors.push(inventor))

//? If  let veryOldInventors = inventors.filter(inventor => inventor.year > 1499 && inventor.year < 1600);
...
firstLast = people.map(people => {
  let stringyName = people.split(', ');
  return `${stringyName[0]} ${stringyName[1]}`;
})
